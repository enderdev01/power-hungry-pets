...
**M8 delivery boundary.** Milestone 8 ships this gameplay layer as a responsive, CSS-only Digital Tabletop Arcade: 63:88 stamped card placeholders, compact public seats, a centered projection-confirmed action stage, textual state indicators, and reduced-motion-safe cues. Configurable asset keys remain the seam for future authorized artwork; M8 intentionally adds no final artwork, textures, branded frames, or background art. The untracked `docs/10_VISUAL_SYSTEM.md`, `docs/11_CARD_COMPONENT_SPEC.md`, and `docs/12_TABLE_SCENE_SPEC.md` files are exploratory drafts, not project authority, and are superseded wherever they conflict with this document or the shipped implementation.
