...
describe('player token rack', () => {
  it('renders the committed count as readable text, singular and plural', () => {
    ...
    expect(screen.getByText('1 victory token')).toBeInTheDocument();

    rerender(<PlayerTokenRack name="Ana" count={3} />);
    expect(screen.getByText('3 victory tokens')).toBeInTheDocument();
    expect(tokenLabel(3)).toBe('3 victory tokens');
    expect(tokenLabel(0)).toBe('0 victory tokens');
  });

  it('renders one labelled slot per committed token', () => {
    render(<PlayerTokenRack name="Ana" count={3} />);
    const rack = screen.getByRole('group', { name: 'Ana: 3 victory tokens' });
    const slots = within(rack).getAllByRole('listitem', { hidden: true });
    expect(slots).toHaveLength(3);
    expect(within(rack).getByText('Victory token 1')).toBeInTheDocument();
    expect(within(rack).getByText('Victory token 3')).toBeInTheDocument();
  });
  ...
  it('announces itself exactly once — one coherent label, no repeated verbose tree', () => {
    ...
    expect(rack).toHaveTextContent('Victory token 1');
    expect(rack).toHaveTextContent('3 victory tokens');
  });

  it('never renders slots or an optimistic increment for an empty count', () => {
    render(<PlayerTokenRack name="Ana" count={0} />);
    const rack = screen.getByRole('group', { name: 'Ana: 0 victory tokens' });
  ...
  it('never renders raw ids, instance ids, or invented state', () => {
    ...
    expect(screen.getByRole('group', { name: 'Ana: 2 victory tokens' })).toBeInTheDocument();
